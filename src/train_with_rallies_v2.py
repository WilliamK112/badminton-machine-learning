...
rally_file = ROOT / "data" / "rally_labels_v2.csv"
...
for rr in rallies:
    ...
    for f in range(ef, sf - 1, -1):
        ...
        if x is None or y is None:
            ...
            if xy is not None:
                x, y = xy[0], xy[1]
        if vis and x is not None and y is not None:
            rec = cand
            break
    if rec is None:
        continue
...
X = np.array(X, dtype=float) if X else np.empty((0, 8), dtype=float)
y_xy = np.array(y_xy, dtype=float) if y_xy else np.empty((0, 2), dtype=float)
y_w = np.array(y_w, dtype=int) if y_w else np.empty((0,), dtype=int)
...
report = {
    ...
    "samples": int(len(X)),
    "note": "rally-aware training on v2 segmentation labels",
    "compare_vs_v4": {
        "v4_winner_acc": base.get("winner_acc"),
        "v4_landing_rmse": base.get("landing_rmse"),
        "delta_winner_acc": None,
        "delta_landing_rmse": None,
    },
}
...
if len(X) >= 8:
    ...
    report["landing_rmse"] = float(rmse)
    report["winner_acc"] = float(acc)
    if base.get("winner_acc") is not None:
        report["compare_vs_v4"]["delta_winner_acc"] = float(acc) - float(base["winner_acc"])
    if base.get("landing_rmse") is not None:
        report["compare_vs_v4"]["delta_landing_rmse"] = float(rmse) - float(base["landing_rmse"])
else:
    report["status"] = "insufficient_rally_samples"
    report["required_min_samples"] = 8
    report["suggestion"] = "Increase segmentation sensitivity or add more video/manual rally labels."

out = ROOT / "reports" / "rally_metrics_v2_compare.json"
