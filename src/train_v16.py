#!/usr/bin/env python3
"""Train v16: Add stance features to angular velocity + endpoint + motion features"""
import json
import gzip
import pandas as pd
import numpy as np
...
# Load quant features (angular velocity)
...
for _, row in rally_v9.iterrows():
    ...
    # Motion features
    avg_motion = row.get('avg_motion', 0)
    max_motion = row.get('max_motion', 0)
    frame_count = row.get('frame_count', end - start)
...
report = {
    ...
    'step_executed': 'Train v16 - Add stance features to angular + endpoint + motion',
    'samples': len(df),
    ...
    'next_step': 'Compare with v15, try different model architectures (RF, XGBoost)'
}

out_path = ROOT / 'reports' / f"train_v16_{datetime.now().strftime('%Y-%m-%dT%H-%M-%S')}.json"
