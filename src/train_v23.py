#!/usr/bin/env python3
"""Train v23: Same as v19 but with additional shuttle trajectory features"""
import json
import gzip
import pandas as pd
import numpy as np
...
# Load frame features for stance and shuttle
...
# Build rally-level features with shuttle trajectory
features_list = []
for _, row in rally_v9.iterrows():
    ...
    # Get shuttle speed over rally
    shuttle_speeds = rally_frames['shuttle_speed'].dropna() if 'shuttle_speed' in rally_frames.columns else []
...
report = {
    ...
    'step_executed': 'Train v23 - v19 with shuttle trajectory features',
    'samples': len(df),
    ...
    'comparison_to_v19': f'v19 CV=0.733, v23 CV={best_score:.3f}',
    'next_step': 'Try adding more features or data augmentation'
}

out_path = ROOT / 'reports' / f"train_v23_{datetime.now().strftime('%Y-%m-%dT%H-%M-%S')}.json"
