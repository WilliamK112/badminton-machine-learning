...
def resolve_csv_preferring_gz(path_no_gz: Path) -> Path:
    gz = Path(f"{path_no_gz}.gz")
    if gz.exists():
        return gz
    if path_no_gz.exists():
        return path_no_gz
    raise FileNotFoundError(f"Missing input file: {path_no_gz} (or {gz.name})")


def main() -> None:
    root = Path(__file__).resolve().parents[1]
    data_path = resolve_csv_preferring_gz(root / "data" / "quant_features_v6.csv")
    reports = root / "reports"
    reports.mkdir(parents=True, exist_ok=True)

    df = pd.read_csv(data_path)
    if df.empty:
        raise RuntimeError("quant_features_v6.csv(.gz) is empty")
    ...
    plt.title("Win-Probability Timeline (v6 quant features)")
    plt.legend(loc="upper right")
    plt.tight_layout()
    wp_path = reports / "win_prob_timeline_v3.png"
    ...
    bins = 80
    h, *_ = np.histogram2d(lx, ly, bins=bins, range=[[0, 1], [0, 1]])
    ...
    plt.title("Landing Heatmap (v6, low-speed proxy)")
    plt.tight_layout()
    hm_path = reports / "landing_heatmap_v3.png"
    ...
    meta = reports / f"visual_reports_v3_{datetime.now().strftime('%Y-%m-%dT%H-%M-%S')}.json"
