#!/usr/bin/env python3
"""
train_with_rallies_v7.py - Rally-aware training with motion quantification features
                          Adds angular velocity features from quantify_motion_v6.py
"""
import csv
import gzip
import json
...
feat_file = ROOT / "data" / "frame_features_v9.jsonl"
...
if not rally_file.exists():
    rally_file = resolve_input("rally_labels_v3.csv")

out_file = ROOT / "reports" / f"rally_metrics_v7_{datetime.now().strftime('%Y-%m-%dT%H-%M-%S')}.json"
...
# Load motion quantification features (angular velocity)
motion_file = ROOT / "data" / "quant_features_v9.csv.gz"
motion_by_frame = {}
if motion_file.exists():
    with gzip.open(motion_file, "rt") as f:
        reader = csv.DictReader(f)
        for row in reader:
            frame = int(row.get("frame", 0))
            # Extract angular velocity features for both players
            motion_by_frame[frame] = {
                "X_arms_ang_vel": safe_float(row.get("X_arms_ang_vel")),
                "X_torso_ang_vel": safe_float(row.get("X_torso_ang_vel")),
                "X_legs_ang_vel": safe_float(row.get("X_legs_ang_vel")),
                "Y_arms_ang_vel": safe_float(row.get("Y_arms_ang_vel")),
                "Y_torso_ang_vel": safe_float(row.get("Y_torso_ang_vel")),
                "Y_legs_ang_vel": safe_float(row.get("Y_legs_ang_vel")),
            }
    print(f"Loaded motion features for {len(motion_by_frame)} frames")
else:
    print("Warning: motion features file not found, skipping angular velocity features")
...
for rr in rallies:
    ...
    # Motion feature collection lists
    motion_X_arms, motion_X_torso, motion_X_legs = [], [], []
    motion_Y_arms, motion_Y_torso, motion_Y_legs = [], [], []
    ...
    for rec in early:
        ...
        # Collect motion features from this frame
        frame_num = rec.get("frame", 0)
        m = motion_by_frame.get(frame_num, {})
        motion_X_arms.append(m.get("X_arms_ang_vel", 0.0))
        motion_X_torso.append(m.get("X_torso_ang_vel", 0.0))
        motion_X_legs.append(m.get("X_legs_ang_vel", 0.0))
        motion_Y_arms.append(m.get("Y_arms_ang_vel", 0.0))
        motion_Y_torso.append(m.get("Y_torso_ang_vel", 0.0))
        motion_Y_legs.append(m.get("Y_legs_ang_vel", 0.0))
    ...
    # Add motion quantification features (angular velocity)
    feat.update(agg_stats("mot_X_arms", motion_X_arms))
    feat.update(agg_stats("mot_X_torso", motion_X_torso))
    feat.update(agg_stats("mot_X_legs", motion_X_legs))
    feat.update(agg_stats("mot_Y_arms", motion_Y_arms))
    feat.update(agg_stats("mot_Y_torso", motion_Y_torso))
    feat.update(agg_stats("mot_Y_legs", motion_Y_legs))
