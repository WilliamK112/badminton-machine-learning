...
def main() -> None:
    ...
    feat_file = resolve_input("frame_features_v10.jsonl")
    out_csv_gz = DATA / "quant_features_v10.csv.gz"
    out_report = REPORTS / f"motion_quantification_v7_{datetime.now().strftime('%Y-%m-%dT%H-%M-%S')}.json"
    ...
    stats = {"X": {"arms": [], "torso": [], "legs": []}, "Y": {"arms": [], "torso": [], "legs": []}}
    ...
    with gzip.open(out_csv_gz, "wt", encoding="utf-8", newline="") as f:
        ...
        for r in rows:
            ...
            row["winner_proxy"] = 1 if y > 0.5 else 0
    ...
    report = {
        ...
        "step_executed": "Reduce storage/runtime (priority #6): quantification now supports .jsonl/.jsonl.gz input and writes compressed-only .csv.gz output",
        "inputs": str(feat_file.relative_to(ROOT)),
        "artifacts": [str(out_csv_gz.relative_to(ROOT)), str(out_report.relative_to(ROOT))],
        "frames": len(rows),
        "summary": {
            "X": {
        ...
        "next_step": "Apply the same compressed-input/output pattern to rally segmentation scripts to eliminate remaining plain CSV duplicates.",
    }

    out_report.write_text(json.dumps(report, indent=2), encoding="utf-8")
    print(str(out_csv_gz))
