...
in_path = ROOT / 'data' / 'frame_features_v2.jsonl'
out_path = ROOT / 'data' / 'frame_features_v3.jsonl'
report_path = ROOT / 'reports' / f'feature_extraction_upgrade_v3_{datetime.now().strftime("%Y-%m-%dT%H-%M-%S")}.json'

rows = [json.loads(x) for x in in_path.read_text().splitlines() if x.strip()]

# temporal fill settings
max_player_gap = 3
max_shuttle_gap = 2
alpha = 0.35  # shuttle EMA smoothing
...
for i, r in enumerate(rows):
    ...
    for side in ['X', 'Y']:
        if players.get(side) is None:
            if last_player[side] is not None and (i - last_player_seen[side]) <= max_player_gap:
                players[side] = deepcopy(last_player[side])
                players[side]['filled'] = True
...
report = {
    ...
    'changes': {
        'player_X_filled_frames': filled_players['X'],
        'player_Y_filled_frames': filled_players['Y'],
        'shuttle_smoothed_frames': smoothed_shuttle,
        'shuttle_interpolated_frames': interpolated_shuttle,
