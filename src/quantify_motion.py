...
out_csv = ROOT / "data" / "quant_features.csv"
out_report = ROOT / "reports" / "motion_quantification_2026-03-24T03-30.json"

rows = [json.loads(x) for x in feat_file.read_text().splitlines() if x.strip()]

# YOLO pose keypoint indices (COCO)
...
fields = [
    ...
    "X_l_calf",
    "X_r_calf",
    # X angular velocity summaries (deg/s equivalent for angle features)
    "X_arms_ang_vel",
    "X_torso_ang_vel",
    "X_legs_ang_vel",
    # Y angles
...
report = {
    ...
    "step_executed": "Add motion quantification (arms/torso/legs angular velocity) (priority #2)",
    ...
    "next_step": "Improve rally segmentation (priority #3) using motion + shuttle visibility smoothing.",
}
